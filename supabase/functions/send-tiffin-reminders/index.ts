  ...
  try {
    ...
    for (const user of users) {
      ...
      try {
        ...
        const pushSubscription = user.push_subscription as NotificationPayload;

        if (!pushSubscription) {
          console.log(`Skipping user ${user.user_id} - no push subscription`);
          continue;
        }
        ...
        console.log(
          `Would send notification to user ${user.user_id}:`,
          notificationPayload
        );
